...
namespace stateline
{
  namespace comms
  {
    ...
    HWorker::HWorker(const std::vector<uint>& jobIDs, const WorkerSettings& settings)
      : running_(true)
    {
      // Setup sockets 
      ...
      VLOG(1) << "Worker binding to minion socket" << HWORKER_SOCKET_ADDR;

      minion->bind(HWORKER_SOCKET_ADDR.c_str());
