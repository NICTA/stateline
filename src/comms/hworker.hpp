...
namespace stateline
{
  namespace comms
  {
    ...
    // const std::string HWORKER_SOCKET_ADDR = "ipc:///tmp/sl_minion.socket";
    const std::string HWORKER_SOCKET_ADDR = "tcp://*:7777";
    ...
    public:
      ...
      //!
      HWorker(const std::vector<uint>& jobIDs, const WorkerSettings& settings);
 
      // Workers can't be copied.
      HWorker(const HWorker &other) = delete;
