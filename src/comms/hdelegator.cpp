...
namespace stateline
{
  namespace comms
  {
    HDelegator::HDelegator(const std::string& commonSpecData, 
          ...
          running_(true)
    {
      ...
      requester->bind(HDELEGATOR_SOCKET_ADDR.c_str());
      ...
      LOG(INFO)<< "HDelegator listening on " << address;
      ...
      auto fInit = std::bind(&HDelegator::sendWorkerProblemSpec, this, ph::_1);
      auto fConnect = std::bind(&HDelegator::connectWorker, this, ph::_1);
      auto fNewJob = std::bind(&HDelegator::newJob, this, ph::_1);

      auto fDisconnect = std::bind(&HDelegator::disconnectWorker, this, ph::_1);
      auto fSwapJobs = std::bind(&HDelegator::jobSwap, this, ph::_1);
      auto fSendFailed = std::bind(&HDelegator::sendFailed, this, ph::_1);
    ...
    void HDelegator::sendWorkerProblemSpec(const Message& msgHelloFromWorker)
    {
    ...
    void HDelegator::connectWorker(const Message& msgJobRequestFromMinion)
    {
    ...
    void HDelegator::sendJob(const Message& msgRequestFromMinion)
    {
      VLOG(1) <<  "Sending a job to the minion..";
      uint id = detail::unserialise<std::uint32_t>(msgRequestFromMinion.data[0]);
      
      VLOG(1) <<  "Minion has requested job of type " << id;

      std::deque<Message>& queue = jobQueues_[id];
      if (!queue.empty())
      {
        VLOG(1) <<  "Queue has work, popping off the top";
      ...
      {
        VLOG(1) <<  "Queue has no work, adding to request queue";
        // Add the minion to the request queue
        requestQueues_[id].push_back(msgRequestFromMinion.address);
      }
    }

    void HDelegator::newJob(const Message& msgJobFromRequester)
    {
      VLOG(1) << "new job request received and being processed";
      uint id = detail::unserialise<std::uint32_t>(msgJobFromRequester.data[0]);
      VLOG(1) << "job has type " << id;
      ...
      if (!queue.empty())
      {
        ...
        // send the job
        VLOG(1) << "sending job to waiting minion";
        router_.send(SocketID::NETWORK, r);
        // add to WIP list
      ...
      {
        VLOG(1) << "pushing job onto queue";
        jobQueues_[id].push_back(msgJobFromRequester);
      }
    }

    void HDelegator::disconnectWorker(const Message& goodbyeFromWorker)
    {
      //most recently appended address
    ...
    void HDelegator::sendFailed(const Message& msgToWorker)
    {
    ...
    void HDelegator::jobSwap(const Message& msgResultFromMinion)
    {
    ...
    std::vector<JobID> HDelegator::jobs() const
    {
      std::vector<JobID> jobs;
